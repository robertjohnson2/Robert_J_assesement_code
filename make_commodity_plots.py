...
def add_analysed_cols(df):
    df["cost"] = 1/df["amount"]
    df['cost_rolling'] = df.groupby('commodity')['cost'].transform(lambda x: x.rolling(7, 1).mean())
    g=df.groupby('commodity').cost
    df['normalised']=(df.cost-g.transform('min'))/g.transform(np.ptp)
    df['normalised_rolling'] = df.groupby('commodity')['normalised'].transform(lambda x: x.rolling(7, 1).mean())
...
def make_mpl_line_plot(df, yvar, commodities, filename, as_percentage=None, label_lines=None):
    ...
    for commodity in commodities:
        ...
        if label_lines is not None:
            ax.text(df[df.commodity==commodity]['date'].iloc[-1], df[df.commodity==commodity][yvar].iloc[-1], commodity)

    ax.axvline(x=INVASION_DATE, c="grey", ls="--", zorder=4)
    ax.text(x=INVASION_DATE, y=ax.get_ylim()[1], s="Russian invasion begins", c="black", ha="right", va="bottom", size=10)
...
df = read_and_merge_commodities(COMMODITIES)
df_processed = add_analysed_cols(df)
df_processed.to_csv("commodities.csv")

df_currencies =  read_and_merge_commodities(CURRENCIES)
df_currency_processed = add_analysed_cols(df_currencies)


make_mpl_line_plot(df_currency_processed, 'perc_change', CURRENCIES, as_percentage=True, filename="currencies")
make_mpl_line_plot(df_processed, 'perc_change', COMMODITIES, as_percentage=True, filename="commodities_perc_chnage", label_lines=True)
make_mpl_line_plot(df_processed, 'normalised_rolling', COMMODITIES, filename="commodities_normalised", label_lines=True)
